...
This module allows users to query a domain to WHOIS info.

Part of IP2Location.io API subscription at https://www.ip2location.io
-}
module DomainWhois (WhoisResult(..), ResponseObj(..), ErrorObj(..), ErrorInfo(..), Registrar(..), Contact(..), lookUpDomain, getDomainName, getDomainExtension) where
...
data WhoisResult
    = WhoisResponse ResponseObj
    | WhoisError ErrorObj
    deriving (Show, Generic)

instance FromJSON Registrar
instance FromJSON Contact
instance FromJSON ResponseObj
instance FromJSON ErrorInfo
instance FromJSON ErrorObj
instance FromJSON WhoisResult where
    parseJSON v = 
        (WhoisResponse <$> parseJSON v) <|> (WhoisError <$> parseJSON v)

{-|
    The 'lookUpDomain' function returns an WhoisResult containing WHOIS data for a domain
    It takes 2 arguments; the API configuration, domain
-}
lookUpDomain :: Config -> String -> IO WhoisResult
lookUpDomain myconfig domain = do
    let format = "json"
    let mySource = source myconfig
    let myVersion = version myconfig
    let myKey = apiKey myconfig

    let url = "https://api.ip2whois.com/v2?key=" ++ (URIE.encode myKey) ++ "&source=" ++ mySource ++ "&source-version=" ++ myVersion ++ "&format=" ++ format ++ "&domain=" ++ (URIE.encode domain)

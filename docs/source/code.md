# IP2Location.io Haskell API

## ip2locationio Module

```{py:function} open(APIKey)
...
```{py:function} lookUpIP(Config, IPAddress, Lang)
...
|proxy.is_ai_crawler|boolean|AI Crawlers.|
...
```{py:function} lookUpDomain(Domain)
